...
Future<void> deleteInterventoById(Intervento intervento) async {
  try {
    ...
    // Controllo se numDoc non è null e quindi elimina anche nel ERP
  ...
  try {
    ...
    await erpdb.writeAsync((isar) {
      interventi.forEach((intervento) {
        isar.interventos.delete(intervento.id ?? 0); // Supponendo che 'id' sia il campo chiave per l'oggetto Intervento in ISAR
      });
    });
